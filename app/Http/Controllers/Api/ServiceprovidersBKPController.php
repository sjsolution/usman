...
class ServiceprovidersBKPController extends Controller
{
  ...
  public function getServiceProviders(Request $request)
  {
    ...
    $service_subquery = $this->service->groupBy('user_id')
    ...
    $rating_subquery = $this->rating->groupBy('service_provider')
          ->selectRaw('service_provider as service_provider_id,AVG(rating) as sp_rating');
    ...
    $providers = User::where('user_type','1')->where('is_active',1)->where('is_service_active',1)
      ...
      ->join(DB::Raw("({$subQuery->toSql()}) AS technician_availibility"),'technician_availibility.tech_userid', '=', 'users.id')
      ->groupBy('users.id')
      ...
      ->addSelect('technician_availibility.technician','users.*','freelancer_services.min_service_price as min_price','service_provider_ratings.sp_rating');
      ...
      if($request->has('filter') && $request->filter == 1){
        ...
        $providers = $providers->where('service_provider_ratings.sp_rating','<', $rating)
      ...
      if($providers->count()){
        ...
        $providers = User::where('user_type','1')->where('is_active',1)
          ...
          ->addSelect('users.*','freelancer_services.min_service_price as min_price','service_provider_ratings.sp_rating');
