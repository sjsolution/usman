...
class ReviewController extends Controller
{
    ...
    {
        $ratings = $this->ratings->where('service_provider',Auth::user()->id)->with('order','user')->orderBy('id','desc')->get();
        ...
        return Datatables::of($ratings)
            ...
            ->addColumn('action',function($ratings){
               return '<button class="btn btn-gradient-danger btn-sm"  data-toggle="tooltip" data-placement="top" title="Review Details" onclick="reviewDetails(event,'.$ratings->id.')"><i class="fa fa-eye"></i></button>';
            })
            ->make(true);  
    }
