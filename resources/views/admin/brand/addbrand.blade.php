    ...
    <div class="card">
        ...
        <h3 class="page-title">{{ $title }}</h3>
            ...
            <?php if(!empty ($brand->id)){ ?>
                <form class="forms-sample" id="formdata" action="{{route('vehicle.updatebrand',$brand->id)}}" data-validate-parsley  method="post"
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Name </label>
                            <input type="text" maxlength="20" class="form-control name_en" name="name_en" value="@if($brand !=''){{ $brand->name_en }}@endif" placeholder="Name"
                                required=" ">
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text"  maxlength="20" class="form-control rtl"
                                name="name_ar" placeholder="الاسم" required=" " value="@if($brand !=''){{ $brand->name_ar }}@endif">
                        </div>
                                ...
                                <select name="vehicle" class="form-control" required=" ">
                                  <option value="">Select vehicles</option>
                                    @foreach ($vehicles as $key => $value)
                                      <option value="{{ $value['id'] }}" @if(!empty($brand->vehicle_type_id) && $brand->vehicle_type_id == $value['id']){{ "selected" }}@endif>{{ $value['name_en'] }}</option>
                                    @endforeach
                          ...
                          <div class="form-group">
                                  ...
                                  <select name="is_active" class="form-control" required=" ">
                                      @foreach ($statusdata as $key=>$status)
                                        <option value="{{ $key }}" @if(!empty($brand->is_active) && $brand->is_active == $key){{ "selected" }}@endif>{{ $status }}</option>
                                      @endforeach
