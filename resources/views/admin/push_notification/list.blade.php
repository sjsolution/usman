...
@section('content')
    ...
    <div class="card">
        ...
        <h3 class="page-title "> User push notification list
            <a  href="{{ route('push.send.form') }}" class="btn btn-gradient-info" style="float:right !important;" >Add Notification</a>
        </h3>
            </div>
        <br> 
...
$(function() {
    ...
    $('#users-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{!! route('push.send.list.view') !!}',
        "columns": [
   ...
   swal({
       ...
       buttons: {
         ...
         if (willDelete) {
           ...
           var $button = $(this);
           var table = $('#users-table').DataTable();
         //  var couponRequest = fetch('http://127.0.0.1:8000/admin/setting/push-notification/send');
           var couponRequest = fetchRequest('/admin/setting/push-notification/send');
...
function deleteNotification(event,id)
{
  ...
  swal({
      title: 'Do you want permanently delete this?',
      text: "",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3f51b5',
      ...
      buttons: {
        ...
        if (willDelete) {
          var $button = $(this);
          var table = $('#users-table').DataTable(); 
          var delVendorRequest = fetchRequest("{{route('sp.push.send.delete')}}/"+id);
          $.get("{{route('sp.push.send.delete')}}/"+id,function(response){
