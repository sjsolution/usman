...
<div class="card">
    <div class="card-body">
        <h3 class="page-title "> Roles List
            <a  href="{{ route('role.management') }}" class="btn btn-gradient-info" style="float:right !important;" >Add Role</a>
   ...
   swal({
       ...
       buttons: {
         ...
         if (willDelete) {
           var $button = $(this);
           var table = $('#users-table').DataTable(); 
           var couponRequest = fetchRequest('status');
           ...
           couponRequest.post().then(function (response) {
               if (response.status === 200) {
                   table.row( $button.parents('tr') ).remove().draw();
                   showInfoToast('Success','Role status successfully changed');
