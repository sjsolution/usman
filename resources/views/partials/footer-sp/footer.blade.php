<footer class="footer">
  <div class="d-sm-flex justify-content-center justify-content-sm-between">
    <span class="text-muted text-center text-sm-left d-block d-sm-inline-block">Copyright © {{date('Y')}} <a href="javascript:void(0)" target="_blank">Maak</a>. All rights reserved.</span>
...
{
    var notificationRenderRequest = fetchRequest('/serviceprovider/notification/order/request/render');
...
function notificationSeenStatus(type){
    ...
    var notificationSeenRequest = fetchRequest('notification/seen/status');
...
function isClickedNotification(event,id){
    ...
    var notificationClickedRequest = fetchRequest('notification/is_clicked');
    ...
    notificationClickedRequest.post().then(function (response) {
        ...
        if (response.status === 200) {
            window.location.href = '/serviceprovider/booking-view'
        }
    });
}
...
{% if(o.list.length) { %}
    <li class="nav-item dropdown" >
        ...
        {% for(var i in o.list){
            var item=o.list[i];              
        %} 
            <a class="dropdown-item preview-item"  onclick="isClickedNotification(event,{%=item.id%});" href="#" style="background:{% if(item.notification_status[0].sp_is_clicked){ %} lightgrey {% } %};">
        ...
        {% } %}
        <div class="dropdown-divider"></div>
        <a href="{{ route('sp.notifications') }}"><h6 class="p-3 mb-0 text-center">See all notifications</h6></a>
...
<!-- start webpushr code --> 
<script>
(function(w,d, s, id) {if(typeof(w.webpushr)!=='undefined') return;w.webpushr=w.webpushr||function(){(w.webpushr.q=w.webpushr.q||[]).push(arguments)};var js, fjs = d.getElementsByTagName(s)[0];js = d.createElement(s); js.id = id;js.src = "https://cdn.webpushr.com/app.min.js";fjs.parentNode.appendChild(js);}(window,document, 'script', 'webpushr-jssdk'));webpushr('init','BG2VQdtugyS8G_qmeTAENiNAjRpO7_pwHzGND_6a5U6pPsmHj3WpxJtsyVWdIvYhdFN50nEil_Qu_Sa49Y2lBA4');
</script>
<!-- end webpushr code -->
